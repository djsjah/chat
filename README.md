...
Для проекта настроен CI/CD pipeline на **GitHub Actions**.
...
На этапе **CI** выполняются следующие проверки:

#### 1. Сборка приложения
Выполняется Gradle-сборка проекта:
...
- кодогенерация OpenAPI выполняется успешно
- generated-код корректно участвует в сборке

Так как в проекте генерация OpenAPI встроена в процесс компиляции, успешный `build` также означает, что:
- файл `src/main/resources/static/openapi.yml` корректен
- `openApiGenerate` успешно отрабатывает
- generated API/interfaces/models успешно компилируются вместе с остальным приложением

#### 2. Сборка исполняемого jar-файла
Дополнительно выполняется:

`./gradlew bootJar`

Эта проверка подтверждает, что приложение может быть собрано в исполняемый Spring Boot jar.

#### 3. Проверка запуска приложения
После сборки CI запускает приложение из собранного jar-файла и проверяет доступность endpoint:

`/actuator/health`

Эта проверка подтверждает, что:
- Spring Boot приложение успешно стартует
- embedded Tomcat успешно поднимается
- приложение начинает слушать HTTP-порт
- Actuator доступен после старта

#### 4. Проверка базы данных и миграций
Во время CI автоматически поднимается временный контейнер **PostgreSQL**.

После запуска приложения проверяется, что:
- приложение успешно подключается к PostgreSQL
- Liquibase успешно выполняет миграции
- схема базы данных может быть инициализирована в чистом окружении

Таким образом, CI проверяет не только компиляцию, но и корректность запуска приложения с базой данных.

#### 5. Проверка docker-compose инфраструктуры
В рамках CI также проверяются docker-compose конфигурации:

- `deploy/keycloak/docker-compose.yml`
- `deploy/monitoring/docker-compose.yml`

Сначала выполняется валидация compose-файлов командой `docker compose config`, после чего проверяется запуск инфраструктуры.

#### 6. Проверка Keycloak-инфраструктуры
В CI поднимается стек из `deploy/keycloak/docker-compose.yml`, содержащий:
- Keycloak
...
- доступность realm endpoint Keycloak

#### 7. Проверка monitoring-инфраструктуры
В CI поднимается стек из `deploy/monitoring/docker-compose.yml`, содержащий:
- Prometheus
- Grafana
- Loki
- Tempo
...
Проверяется:
- корректность запуска контейнеров monitoring-стека
- readiness Prometheus
- readiness Loki
- readiness Tempo
- health Grafana
- факт запуска контейнера Alloy

#### 8. Проверка интеграции приложения с Prometheus
После запуска monitoring-стека CI дополнительно проверяет, что **Prometheus видит `chat-service` как доступный target**.

Это подтверждает, что:
- endpoint `/actuator/prometheus` доступен
- Prometheus успешно скрапит метрики приложения
- конфигурация monitoring-стека корректна

### Что делает CD

На этапе **CD** после успешного прохождения CI:
- собирается исполняемый Spring Boot jar
- из каталога сборки удаляется `plain.jar`
- итоговый jar загружается как build artifact GitHub Actions

Артефакт публикуется в интерфейсе GitHub Actions и может быть скачан после успешного выполнения workflow.

### Build artifact

В качестве артефакта CD публикуется:

`chat-service-deployable`

Это исполняемый Spring Boot jar-файл приложения.

### Где смотреть результаты CI/CD

Все результаты выполнения pipeline доступны во вкладке **Actions** в GitHub-репозитории. 

**Важно:** Более ранние workflow runs относятся к промежуточным этапам настройки pipeline и могут содержать неполный набор проверок.

Во вкладке **Actions** можно посмотреть:
- статус выполнения workflow
- статус каждого job и каждого step
- логи сборки
- логи запуска приложения
- логи проверки инфраструктуры
- причину ошибки при падении pipeline
- опубликованный jar-артефакт

### Примечания

- В CI используются временные контейнеры и временное окружение GitHub Actions.
- PostgreSQL, Keycloak и monitoring stack поднимаются только на время выполнения pipeline.
- После завершения workflow временное окружение автоматически удаляется.
- CI/CD в данном проекте проверяет не только сборку, но и базовую работоспособность приложения и инфраструктуры в чистом окружении.
